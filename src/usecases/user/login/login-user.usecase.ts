...
import { CredentialsNotValidUsecaseException } from "src/usecases/exceptions/user/credentials-not-valid.usecase.exception";
...
export abstract class LoginUserUsecase
  implements Usecase<LoginUserInput, LoginUserOutput> {
    ...
    public async execute({ email, password }: LoginUserInput): Promise<LoginUserOutput> {
